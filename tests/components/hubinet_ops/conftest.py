...
def auto_enable_custom_integrations(enable_custom_integrations: None) -> None:
    """Allow Home Assistant to load Hubinet-Ops as a custom integration."""
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "custom_components.hubinet_ops.async_setup_entry",
        return_value=True,
    ) as mock_setup:
        yield mock_setup
...
def mock_proxmox_client():
    ...
    with (
        patch(
            "custom_components.hubinet_ops.coordinator.ProxmoxAPI", autospec=True
        ) as mock_api,
        patch("custom_components.hubinet_ops.config_flow.ProxmoxAPI") as mock_api_cf,
    ):
