...
case "${1-}" in
  ...
  --help)
    ...
    printf '%s\n' "  --reset  Rotate only Hubinet-owned enrollment credentials."
    exit 0
    ;;
  *)
...
if [ ! -d /etc/pve ] || [ ! -e /etc/pve/local ]; then
...
# Inspect every fixed Hubinet-owned PVE resource before any write. In
# particular, a token collision in normal mode must result in zero mutation.
...
if [ -e "$AUTHORIZED_KEYS2" ] || [ -L "$AUTHORIZED_KEYS2" ]; then
  if [ ! -f "$AUTHORIZED_KEYS2" ] || [ -L "$AUTHORIZED_KEYS2" ]; then
    fail "authorized_keys2 is not a regular file"
  fi
  python3 - "$AUTHORIZED_KEYS2" <<'PY' || fail "authorized_keys2 contains keys not owned by Hubinet-Ops"
import re
import sys
...
if any(not pattern.fullmatch(line) for line in lines) or len(lines) > 1:
    raise SystemExit(1)
PY
fi
...
if [ "$TOKEN_EXISTS" -eq 1 ] && [ "$RESET" -eq 0 ]; then
  printf '%s\n' "Hubinet-Ops enrollment credentials already exist; nothing was changed." >&2
  printf '%s\n' "To explicitly rotate only Hubinet-owned credentials, run:" >&2
...
if [ "$HELPER_EXACT" -eq 0 ]; then
  ...
  HELPER_INSTALL=$(mktemp /usr/local/sbin/.hubinet-package-scan-helper.XXXXXX) || fail "could not stage the helper"
  install -o root -g root -m 0755 "$HELPER_DOWNLOAD" "$HELPER_INSTALL"
  notice "installing ${HELPER_TARGET}"
...
printf 'restrict,command="%s" %s\n' "$HELPER_TARGET" "$PUBLIC_KEY" > "$AUTHORIZED_TEMP"
install -o root -g root -m 0600 "$AUTHORIZED_TEMP" "$AUTHORIZED_KEYS2"
notice "installed the restricted Hubinet key in ${AUTHORIZED_KEYS2}"
...
# Prove enrollment construction before touching the non-recoverable token secret.
printf 'preflight-only\n' > "$TOKEN_SECRET_FILE"
build_enrollment >/dev/null || fail "could not prepare enrollment output"

# Token removal/creation is deliberately last. PVE reveals the new secret once.
if [ "$TOKEN_EXISTS" -eq 1 ]; then
  notice "--reset requested: rotating only token ${PVE_USER}!${PVE_TOKEN_ID}"
...
printf '\nHUBINET enrollment value (paste this into Home Assistant):\n%s\n' "$ENROLLMENT"
